...
void QCPAxis::setupTickVectors()
{
  ...
  if (mAutoTickLabels)
  {
    ...
    else if (mTickLabelType == ltDateTimeMsec)
    {
      for (int i=mLowestVisibleTick; i<=mHighestVisibleTick; ++i)
      {
        mTickVectorLabels[i] = mParentPlot->locale().toString(QDateTime::fromMSecsSinceEpoch(mTickVector.at(i)).toTimeSpec(mDateTimeSpec), mDateTimeFormat);
      }
    }
