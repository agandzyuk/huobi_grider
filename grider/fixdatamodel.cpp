...
namespace {
    string parse52TimeDiff(const string& tag52stamp)
    {
        ...
        char tmp[20] = {0};
        if( diffSecs > 86400 ) {
            _ltoa_s(qint32(diffSecs/86400),tmp,10);
            report += tmp;
            report += "d ";
            diffSecs %= 86400;
        }

        sprintf_s(tmp, 20, "%.2d:", qint32(diffSecs/3600));
        report += tmp;
        diffSecs %= 3600;

        sprintf_s(tmp, 20, "%.2d:", qint32(diffSecs/60));
        report += tmp;
        diffSecs %= 60;

        sprintf_s(tmp, 20, "%.2d", qint32(diffSecs));
        report += tmp;
...
//////////////////////////////////////////////////////////////
FixDataModel::FixDataModel(QWidget* parent) 
    : SymbolsModel(parent),
